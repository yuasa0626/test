"""
Proposed Portfolio Analysis Page
Displays analysis results for the proposed portfolio (after product replacement).
"""
...
def main():
    ...
    st.title("入れ替え後のポートフォリオ分析")
    st.markdown("検討中のポートフォリオ（商品入れ替え後）の詳細分析")
    ...
    if not st.session_state.proposed_portfolio:
        st.warning("検討中のポートフォリオが設定されていません。入力ページでポートフォリオを設定してください。")
    ...
    portfolio = st.session_state.proposed_portfolio
